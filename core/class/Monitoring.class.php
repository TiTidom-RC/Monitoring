	...
	public function getCmdReplace(string $cmdName, array $cmdOptions, &$replace) {
		...
		$optionActions = [
			...
			'stats' => function() use ($isCmdObject, $cmd, $cmdNamePrefix, $cmdName, &$replace) {
				...
				$replace[$cmdNamePrefix . '_tendance#'] = ($isCmdObject && $this->getConfiguration('stats_tendance', '0') == '1') ? ' <i style="color: var(--al-info-color) !important;" class="fas fa-' . $cmd->getConfiguration($cmdName . '_tendance') . '"></i>' : '<i></i>';
			}
		];
