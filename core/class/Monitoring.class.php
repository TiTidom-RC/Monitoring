...
class Monitoring extends eqLogic {
	...
	public function formatNetwork($_network_txrx, $_network_ip, $_equipement) {
		...
		$network_tx = $network_tx != 0 ? round($network_tx / 1024 / 1024, 2) : 0;
		$network_rx = $network_rx != 0 ? round($network_rx / 1024 / 1024, 2) : 0;
	...
	public function formatSwap($_swap, $_equipement) {
		...
		$swap_total = $swap_total != 0 ? round($swap_total / 1024, 2) : 0;
		$swap_used = $swap_used != 0 ? round($swap_used / 1024, 2) : 0;
		$swap_free = $swap_free != 0 ? round($swap_free / 1024, 2) : 0;
	...
	public function formatMemory($_memory, $_uname, $_equipement) {
		...
		if (!preg_match("#FreeBSD#", $_uname)) {		
			...
			// Convert to Mo, source in Ko. it's to avoid problem with big values in Jeedom History DB
			$memory_total = $memory_total != 0 ? round($memory_total / 1024, 2) : 0;
			$memory_used = $memory_used != 0 ? round($memory_used / 1024, 2) : 0;
			$memory_free = $memory_free != 0 ? round($memory_free / 1024, 2) : 0;
	...
	public function formatHDD($hdd_value, $hdd_name, $equipement) {
		...
		// Convert to Mo, source in Ko. it's to avoid problem with big values in Jeedom History DB
		$hdd_total = $hdd_total != 0 ? round($hdd_total / 1024, 2) : 0;
		$hdd_used = $hdd_used != 0 ? round($hdd_used / 1024, 2) : 0;
		$hdd_free = $hdd_free != 0 ? round($hdd_free / 1024, 2) : 0;
