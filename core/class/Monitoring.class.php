			...
			if ($cnx_ssh != 'KO') {
				...
				if ($cnx_ssh != 'KO') {
					...
					elseif ($ARMv == 'i686' || $ARMv == 'x86_64' || $ARMv == 'i386'){
						...
						if ($cpufreq == '') {
							$cpufreqVMbis_cmd = "lscpu | grep '^CPU max MHz' | awk '{ print $NF }'";	// OK pour LXC Linux, Proxmox
		...
		if (($confLocalOrRemote == 'deporte' || $confLocalOrRemote == 'deporte-key') && $this->getIsEnable()) {
			...
			if ($cnx_ssh != 'KO') {
				...
				if ($cnx_ssh != 'KO') {
					...
					switch ($paramaction) {
						case "reboot":
							$rebootcmd = "sleep 2; sudo shutdown -r now >/dev/null";
							...
							break;
						case "poweroff":
							$poweroffcmd = 'sleep 2; sudo shutdown -h now >/dev/null';
